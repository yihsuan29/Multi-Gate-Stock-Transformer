...
| Meaning/ Metric            | IC / ICIR                             | RankIC / RankICIR                     | Excess Return / IR                  |
|----------------------------|---------------------------------------|---------------------------------------|-------------------------------------|
| Type                       | Correlation-based                   | Rank correlation-based                  | Profitability-based                 |
| Interpretation             | linear correlation between predicted and true return| ranking consistency         | absolute and risk-adjusted return |
| Higher is better         | Yes                                   | Yes                                   | Yes                                 |
...
---
### Ablation Study - Beta
![width:1100px center](./Images/AB1.png)X-axis: Temperature parameter β
Y-axis:  Performance metric (e.g., IC or ICIR)

CSI800, a smaller β yields better results
--> stronger gating helps filter out irrelevant features.
...
- **Findings**:
  - Differences among various head combinations are not significant compared to the inherent variance in each setting.
  - Most settings in the studied range outperform the baseline consistently, demonstrating the robustness.
---
### Conclusion

- **MASTER**, a novel Transformer-based model for stock prediction guided by market status.
- We introduce **three key innovations**:
  1. **Market-Guided Gating**  
     Stock features based on market status for feature selection.
  2. **Intra-Stock Aggregation**  
     Temporal dependencies within a single stock for self-attention.

  3. **Inter-Stock Aggregation**  
     Cross-stock correlations for relational structure across stocks.
---
### Conclusion
- Comprehensive experiments on **CSI300 and CSI800** datasets demonstrate that MASTER:

  - Outperforms existing baselines across evaluation metrics
  - Maintains robustness across different market conditions
  - Benefits from flexible and interpretable architecture
