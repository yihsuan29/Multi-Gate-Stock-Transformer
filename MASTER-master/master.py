...
class TAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        dim = int(self.d_model / self.nhead) #256/4
        att_output = []
        for i in range(self.nhead):
            if i==self.nhead-1:
                qh = q[:, :, i * dim:]
...
class MASTER(nn.Module):
    def __init__(self, d_feat, d_model, t_nhead, s_nhead, T_dropout_rate, S_dropout_rate, gate_input_start_index, gate_input_end_index, beta):
        ...
        self.d_gate_input = (gate_input_end_index - gate_input_start_index) # F' 63
    ...
    def forward(self, x):
        ...
        market_src = src * torch.unsqueeze(self.feature_gate(gate_input), dim=1) 
