...
#         self.open = df_data['OPENPRC'] #['S_DQ_OPEN'] 
#         self.high = df_data['ASKHI'] #['S_DQ_HIGH'] 
#         self.low = df_data['BIDLO'] #['S_DQ_LOW']   
#         self.close = df_data['PRC'] #['S_DQ_CLOSE'] 
#         self.volume = df_data['VOL']*100  #['S_DQ_VOLUME']*100 
#         self.returns = df_data['RET'] #['S_DQ_PCTCHANGE'] 
#         self.vwap = (df_data['AMOUNT']*1000)/(df_data['VOL']*100+1) #(df_data['S_DQ_AMOUNT']*1000)/(df_data['S_DQ_VOLUME']*100+1) 
...
#         alpha.loc[cond, 'close'] = (-1 * delta(self.high, 2).fillna(0)).loc[cond]
#         return alpha
...
#     # Create or load your dataframe 'df' with required columns
#     df_all = pd.read_parquet('../Data/CRSP/CRSP_cleaned.pqt') 
#     #tickers = ['GOOGL', 'AMZN', 'MSFT']

#     # Filter the DataFrame to include only the desired tickers
...
#     print(df_all.shape)
#     # Group by PERMNO
#     for permno, group in tqdm(df_all.groupby("PERMNO")):
#         group = group.copy() 
#         get_alpha(group)   
#         group['PERMNO'] = permno
#         alpha_list.append(group)

#     # Concatenate all individual PERMNO groups into a single DataFrame
#     df_with_alphas = pd.concat(alpha_list).sort_values(by=["PERMNO", "date"])
#     print(df_with_alphas.head(5))
#     print(df_with_alphas.shape)
    
#     df_with_alphas.to_parquet('../Data/CRSP/CRSP_alpha.pqt')
import pandas as pd
df = pd.read_parquet('../Data/CRSP/CRSP_alpha.pqt')
for x in df.columns:
    print(x)